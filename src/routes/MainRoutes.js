...
const CreatePost = Loadable(lazy(() => import('pages/board/CreatePost'))); // 작성해주세요.

// [3 게시글 상세] Todo: 게시글 상세(Post) 페이지 컴포넌트 등록
const Post = Loadable(lazy(() => import('pages/board/Post'))); // 작성해주세요.

// [4 게시판 수정] Todo: 게시판 수정(UpdatePost) 페이지 컴포넌트 등록
const UpdatePost = Loadable(lazy(() => import('pages/board/UpdatePost'))); // 작성해주세요.
...
const MainRoutes = {
	...
	children: [
		...
		{
			path: 'post/:id',
			element: <Post />, //엘리먼트 매핑해주세요.
		},
		// [4 게시글 수정] Todo: 게시글 수정(UpdatePost) 페이지 라우팅('post/update')
		{
			path: 'post/update',
			element: <UpdatePost />, //엘리먼트 매핑해주세요.
		}, // 작성해주세요.
	],
};
